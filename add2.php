...
if (!empty($passValueLoginPage)) {
    ...
    <body>
        ...
        if ($_GET['St_email']) {
            ...
            $apply_email = $_GET['St_email'];
            $apply_jobId = $_GET['jobId'];
                ...
                <div class="content">
                    ...
                    if (isset($_POST['confirmT'])) {
                        ...
                        if ($_GET['St_email']) {
                            ...
                            $sql_i = "SELECT* FROM studentloanuserinfo WHERE  email = '$email' " ;
                            $query_i = $conn->query($sql_i);

                            if ($query_i->num_rows> 0) {
                                $data = mysqli_fetch_assoc($query_i);
    
                                $log_loneType = $data['loneType'];
                            }
                            ...
                            if ($query->num_rows> 0) {
                                ...
                                $apply_loneType = $data['loneType'];
                            }
                


                            $sql_in = "INSERT INTO loan_dashboaer
                            (login_email,apply_email,apply_deg, login_deg)
                            VALUES('$email','$apply_email','$apply_loneType', '$log_loneType')";

                            $result_in = $conn->query($sql_in);

                            $sqlup =   "UPDATE allapplyloan 
                            SET decission='$cnfrm' WHERE loginUserEmail ='$apply_email' AND applyPostNum = '$apply_jobId' ";
                            $resultup = $conn->query($sqlup);

                            $sql = "DELETE FROM loanpost WHERE id = '$apply_jobId'";
                            $result = $conn->query($sql);
                             echo "<script>window.location.href ='dashboardLoan.php';</script>";
                    ...
                    if (isset($_POST['noT'])) {
                        ...
                        echo "<script>window.location.href ='requstLoan.php';</script> ";
